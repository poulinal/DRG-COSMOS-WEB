...
class CosmosWebHandler:
    ...
    def __init__(self, path, catalog_type='master'):
        ...
        self.cat_photom : dict[Table] = {}
        self.cat_lephare : dict[Table] = {}
        self.cat_cigale : dict[Table] = {}
        self.cat_bd : dict[Table] = {}
    ...
    def purity_cut(self):
        ...
        self.condition_clean['original'] = np.logical_and.reduce((
        ...
        mask = self.condition_clean['original']
    ...
    def miri_cut(self):
        condition_clean_miri = np.logical_and(self.condition_clean['original'], self.cat_photom['original']['flux_model_f770w']>0)
        self.condition_clean['condition_clean_miri'] = condition_clean_miri

        print(f"MIRI cut: {np.sum(condition_clean_miri)} out of {len(self.cat_lephare['original'])} objects remain. Fraction: {np.sum(condition_clean_miri)/len(self.cat_lephare['original']):.2%}")
    ...
    def get_filter_cut(self, filter_func : Callable | np.ndarray, filtername : str = "default_filter"):
        """
        ...
        if callable(filter_func):
            mask = filter_func(
                self.cat_photom['condition_clean_miri'],
                self.cat_lephare['condition_clean_miri'],
                self.cat_cigale['condition_clean_miri'],
                self.cat_bd['condition_clean_miri'],
            )
        else:
        ...
        print(f"Filter cut: {np.sum(mask)} out of {len(self.cat_lephare['condition_clean_miri'])} objects remain. Fraction: {np.sum(mask)/len(self.cat_lephare['condition_clean_miri']):.2%}")

        self.cat_lephare[filtername] = self.cat_lephare['condition_clean_miri'][mask]
        self.cat_photom[filtername] = self.cat_photom['condition_clean_miri'][mask]
        self.cat_cigale[filtername] = self.cat_cigale['condition_clean_miri'][mask]
        self.cat_bd[filtername] = self.cat_bd['condition_clean_miri'][mask]
